...
def _post_process_report(args, env: dict, report_dir: Path, standalone: Path) -> None:
    ...
    auto_open_report = (
        not getattr(args, "no_open_report", False)
...
def _post_process_direct_report(args, env: dict | None = None) -> None:
    """Resolve and post-process a report returned by a non-single-stock runner."""
    env = env or detect_environment()
    report_dir, standalone = _resolve_report_artifact(args, SCRIPTS_DIR, args.ticker)
    if not standalone.exists():
        raise FileNotFoundError(f"报告文件未找到: {standalone}")
...
def main():
    ...
    if args.versus:
        ...
        if result.get("status") == "completed":
            ...
            args.ticker = "versus"
            _post_process_direct_report(args)
            sys.exit(0)
        else:
            ...
            if status == "non_stock_security":
                ...
                if sec_type in ("etf", "lof", "mutual_fund") and result.get("top_holdings"):
                    ...
                    if fund_result.get("status") == "completed":
                        print(f"\n✅ 持仓批量分析完成 · 汇总报告: {fund_result['summary_html']}")
                        _set_direct_report_path(args, fund_result["summary_html"])
                        _post_process_direct_report(args, env)
                        sys.exit(0)
                    else:
                        sys.exit(0)
                else:
