...
def _fetch_basic_us(ti: TickerInfo) -> dict:
    ...
    out = {
        ...
        "market_cap_yi": round(market_cap_raw / 1e8, 4) if market_cap_raw is not None else None,
        "total_shares": shares,
        "total_shares_source": "yfinance.info.sharesOutstanding" if shares is not None else None,
        "price": price,
...
def _kline_us_chain(ti: TickerInfo) -> list[dict]:
    """Global K-line: yfinance plus a strictly newer Yahoo v8 tail and fallbacks."""
    from .global_peers import to_yahoo_symbol
